...
// vector 안에 우선순위 큐 저장 --> n만큼 큐 생성해서 순서대로 하나씩 집어넣고 top끼리 비교하기
// --> 걍 tuple로 줄 순서까지 다 넣어버림... 그걸 걍 우선순위 큐에 넣고 비교구조체 정의해서 ㄱㄱ

// 튜터님은 struct로 구조체 생성해서 푸심 --> 이게 훨 나은듯;
...
int main() {
    ...
    cin >> n >> m >> k;
    vector<queue<tuple<int, int, int, int>>> line(m);
    tuple<int, int, int, int> deka;

    for (int i=0; i<n; i++) {
        cin >> d >> h;
        line[i%m].push({d, h, i%m, i});
        if (i == k) deka = {d, h, i%m, i};
    }
    
    priority_queue<tuple<int, int, int, int>, vector<tuple<int, int, int, int>>, cmp> toilet;
    for (int i=0; i<m; i++) {
        if (!line[i].empty()) {
            toilet.push(line[i].front());
            line[i].pop();
        }
    }
    ...
    int count = 0;
    while (!toilet.empty()) {
        tuple<int, int, int, int> enter = toilet.top();
        if (enter != deka) {
            count++;
            toilet.pop();
        } else break;
